...
import db_sync
from sync_section import find_latest_panel as _find_latest_section_panel
...
def find_latest_macro_json() -> dict | None:
    """
    在 data/macro/ 中找最新宏观文件并读取。
    优先级：
      1. macro_YYYYMMDD.json 中日期最大的
      2. latest.json 兼容
    返回 dict（无最新则返回 None）
    """
    macro_dir = os.path.join(DATA_DIR, 'macro')
    if not os.path.isdir(macro_dir):
        return db_sync.read_json('macro', 'latest')  # 兜底
    files = [f for f in os.listdir(macro_dir) if f.startswith('macro_') and f.endswith('.json')]
    if files:
        best = None
        for f in files:
            # macro_20260728.json -> 8位日期
            rest = f[len('macro_'):]
            digits = ''
            for c in rest:
                if c.isdigit():
            ...
            if len(digits) >= 8:
                ...
                if best is None or ymd > best:
                    best = ymd
        if best:
            full = os.path.join(macro_dir, f'macro_{best}.json')
            try:
                import json
                ...
                pass
    # 兜底: latest.json
    return db_sync.read_json('macro', 'latest')
...
def load_section() -> pd.DataFrame:
    ...
    panel_path, panel_ymd = _find_latest_section_panel()
    if panel_path is None or not os.path.exists(panel_path):
        ...
        if os.path.exists(bulk_sync_zz500.DEFAULT_CSV):
            ...
            bulk_sync_zz500.bulk_sync_to(today_ymd, bulk_sync_zz500.DEFAULT_CSV, 60)
            panel_path, panel_ymd = _find_latest_section_panel()
        else:
            raise FileNotFoundError(f'缺少截面 parquet 且找不到成分股清单: {bulk_sync_zz500.DEFAULT_CSV}')
    if panel_ymd:
        print(f'  📂 读最新截面 panel: zz500_60d_{panel_ymd}.parquet')
    return pd.read_parquet(panel_path, engine='pyarrow')
...
def build_fund_flow_inputs(std: str, stock_df: pd.DataFrame | None = None):
    ...
    # ── 兜底逻辑 ──────────────────────────────────────────────
    # 若 JumboNetFlow 和 BlockNetFlow 都缺（raw_input 没填），但 MainNetFlow 有值，
    # 则 DDE（特大单+大单）= MainNetFlow。因为 MainNetFlow = 特大单 + 大单。
    _has_jumbo = any(_to_float(r, 'JumboNetFlow') != 0 for r in records)
    _has_block = any(_to_float(r, 'BlockNetFlow') != 0 for r in records)
    _use_mainnet_fallback = (not _has_jumbo and not _has_block
                             and fund_flow['mainNetInflow'] != 0.0)

    if _use_mainnet_fallback:
        fund_flow['hugeNetInflow'] = 0.0
        fund_flow['bigNetInflow'] = fund_flow['mainNetInflow']

    # 最近 3 日 DDE 累计（元）：特大单 + 大单
    last3 = records[-3:]
    if _use_mainnet_fallback:
        fund_flow['dde_3d'] = sum(_to_float(r, 'MainNetFlow') for r in last3)
    else:
    ...
    if _use_mainnet_fallback:
        dde_vals = [_to_float(r, 'MainNetFlow') for r in records]
    else:
