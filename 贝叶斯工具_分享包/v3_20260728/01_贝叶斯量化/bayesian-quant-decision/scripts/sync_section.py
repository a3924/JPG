...
CACHE_FILE = os.path.join(CACHE_DIR, 'zz500_60d.parquet')  # legacy 兼容
CACHE_FILE_TPL = 'zz500_60d_{ymd}.parquet'


def panel_path_for_date(ymd: str) -> str:
    """给定 YYYYMMDD 返回截面文件路径"""
    return os.path.join(CACHE_DIR, CACHE_FILE_TPL.format(ymd=ymd))


def find_latest_panel() -> tuple[str | None, str | None]:
    """
    在 data/section/ 中找最新日期的截面 panel 文件
    优先级：
      1. 带日期的 zz500_60d_YYYYMMDD.parquet（按文件名 YYYYMMDD 排序，desc 取最大）
      2. 兼容 legacy zz500_60d.parquet（按 mtime，格式 YYYYMMDD）
    返回 (file_path, ymd)；找不到返回 (None, None)
    """
    if not os.path.isdir(CACHE_DIR):
        return None, None
    files = [f for f in os.listdir(CACHE_DIR) if f.endswith('.parquet')]
    # 1. 找带日期文件中日期最大的
    best_ymd: str | None = None
    for f in files:
        # 解析格式: zz500_60d_20260728.parquet -> 取 '20260728'
        if not f.startswith('zz500_60d_'):
            continue
        rest = f[len('zz500_60d_'):]
        digits = ''
        for c in rest:
            if c.isdigit():
                digits += c
        ...
        if len(digits) >= 8:
            ymd = digits[:8]
            if best_ymd is None or ymd > best_ymd:
                best_ymd = ymd
    if best_ymd:
        return panel_path_for_date(best_ymd), best_ymd
    # 2. 兼容 legacy
    legacy = 'zz500_60d.parquet'
    if legacy in files:
        return os.path.join(CACHE_DIR, legacy), None
    return None, None


def list_panel_files() -> list[dict]:
    """列出所有截面 panel 文件及对应日期，按日期降序"""
    if not os.path.isdir(CACHE_DIR):
        return []
    out = []
        ...
        if f.startswith('zz500_60d_'):
            ...
            for c in rest:
                if c.isdigit():
                    ...
                    break
            ymd = digits[:8] if len(digits) >= 8 else None
        elif f == 'zz500_60d.parquet':
            ymd = None
        else:
            continue
        out.append({'path': full, 'name': f, 'ymd': ymd,
                    'mtime': datetime.fromtimestamp(os.path.getmtime(full))})
    out.sort(key=lambda r: r['ymd'] or r['mtime'].strftime('%Y%m%d'), reverse=True)
    return out
